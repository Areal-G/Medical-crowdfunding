...
const HospitalAdminPatientsPage = () => {
  const [currentLoadingId, setCurrentLoadingId] = useState(null);
  const [Data, setData] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [individualPatient, setIndividualPatient] = useState(null);

  const handleModalToggle = async (id) => {
    if (!isModalOpen) {
      try {
        const response = await API.get(
          `/hospital/getpatientdataforhospital/${id}`,
        );
        setIndividualPatient(response.data);
        console.log(response.data);
      } catch (error) {
        console.error("Error fetching individual patient data:", error);
      }
    }
  ...
  const fetchData = async () => {
    try {
      const response = await API.get(`/hospital/getpatientstableinhospital`);
  ...
  const handleToggleBlock = async (Id, newStatus) => {
    try {
      ...
      await API.put(`/sysadmin/updatepatientstatus/${Id}`, {
        status: newStatus,
      });
      fetchData();
    } catch (err) {
                  ...
                  <div className="mt-5">
                    ...
                    <div className="">
                      ...
                      <p className="mt-3 ">
                        ...
                        {individualPatient?.patient.city}
                      </p>
                      ...
                      <p className="mt-3  ">
                        ...
                        {individualPatient?.isCampaign ? "Yes" : "No"}
                      </p>
                      <p className="mt-3">
                        <span className=" font-semibold"> Joined : </span>
                        {new Date(
                          individualPatient?.patient.createdAt,
                        ).toLocaleDateString()}
                      </p>
                      <p className="mt-3">
                        ...
                        <span
                          className={`inline-flex rounded-full bg-opacity-10  px-3 py-1 text-sm font-medium uppercase ${
                            individualPatient?.patient.status === "active"
                              ? " bg-[#219653] text-[#219653]"
                              : individualPatient?.patient.status === "blocked"
                                ? "bg-[#D34053] text-[#D34053]"
                                : "bg-[#FFA70B] text-[#FFA70B]"
                          }`}
                        >
                          {individualPatient.patient.status}
                        </span>
                      </p>
                    </div>
                  </div>
...
export default HospitalAdminPatientsPage;
